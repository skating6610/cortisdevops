...
在数据库`cortisdevops-admin`中`cortisdevops_a_users`表字段 superuser
- 0：代表超级管理员
- 10：代表普通用户

## 业务、用户、权限配置
https://github.com/cortisdevops-cn/cortisdevops-admin/blob/main/docs/cortisdevops-admin.md
...
- 点击菜单栏`用户管理`-`用户列表`-`新建用户`， MFA使用参考[FAQ](https://docs.cortisdevops.cn/zh/guide/more/faq/)
![](/create_user01.png)
![](/create_user02.png)
![](/create_user03.png)
![](/create_user04.png)
...
- 绑定DN： `cn=Manager,DC=cortisdevops,DC=cn`，这里是设置认证用户的信息, 系统会使用这个用户去校验ldap的信息是否正确
- 密码： 上面认证用户的密码


- 用户OU： `ou=cortisdevops,dc=cortisdevops,dc=cn`，这里是设置用来登录cortisdevops的组织单元, 比如我要用某个ou的用户来登录cortisdevops
...
- 启动LDAP认证：如果需要使用LDAP登录 cortisdevops, 则必选
