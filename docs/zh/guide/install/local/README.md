...
- 如果你的系统是新的，我们建议你先优化下系统，同样我们也提供了[优化系统脚本](https://github.com/cortisdevops-cn/cortisdevops/tree/master/scripts/system_init_v1.sh)
...
```shell
mkdir -p /opt/cortisdevops/ && cd /opt/cortisdevops/
git clone https://github.com/cortisdevops-cn/cortisdevops-deploy-docs.git
cd cortisdevops-deploy-docs/docker-deploy
```

- 配置文件修改【可选】[.env](https://github.com/cortisdevops-cn/cortisdevops-deploy-docs/blob/main/docker-deploy/.env) <span style="color: red;">
  中已经配置项目中所需的账密信息，不修该则使用默认配置</span>
- 数据库初始化【可选】：[db_init.sql](https://github.com/cortisdevops-cn/cortisdevops-deploy-docs/blob/main/docker-deploy/db_init.sql) <span style="color: red;"> 在 Docker 部署 MySQL 时默认导入。
